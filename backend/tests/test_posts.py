...
@pytest.mark.asyncio
async def test_create_post_empty_content(client: AsyncClient, db_session):
    ac, _, board = await _prepare_user_and_board(client, db_session)
    payload = {"title": "Title", "content": "", "board_id": str(board.id)}
    resp = await ac.post(POSTS_LIST, json=payload)
    assert resp.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


@pytest.mark.asyncio
async def test_create_post_content_too_long(client: AsyncClient, db_session):
    ac, _, board = await _prepare_user_and_board(client, db_session)
